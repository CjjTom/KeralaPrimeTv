...
async def start(client, message):
    if not await checkdb.is_user_exist(message.from_user.id):
        ...
        name = await client.ask(message.chat.id, "<b>Welcome To CJ Disk.\n\nIts Time To Create Account On CJ Disk\n\nNow Send Me Your Business Name Which Show On Website\nEx :- <code>CJ Tom</code></b>")
        if name.text:
...
async def link_start(client, message):
    if not message.text.startswith(LINK_URL):
        return
    link_part = message.text[len(LINK_URL + "?CJ_Tom="):].strip()
    try:
